 ...
 * SPM Sync Handler
 *
 * Handles data synchronization from SPM Dashboard API
 *
 * @package SPM_Sync
 */
...
class SPM_Sync {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_spm_manual_sync', array($this, 'ajax_manual_sync'));
        add_action('wp_ajax_spm_check_version', array($this, 'ajax_check_version'));
    }
    ...
    private function get_dashboard_url() {
        return rtrim(get_option('spm_dashboard_url', ''), '/');
    }
    ...
    private function api_request($endpoint, $params = array()) {
        ...
        if (empty($dashboard_url) || empty($api_key)) {
            return new WP_Error('missing_config', __('API configuration is missing', 'spm-sync'));
        }
        ...
        if ($status_code !== 200) {
            return new WP_Error(
                'api_error',
                sprintf(__('API returned status %d', 'spm-sync'), $status_code)
            );
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            return new WP_Error('json_error', __('Invalid JSON response', 'spm-sync'));
        }
    ...
    public function sync_all_data($types = null) {
        ...
        foreach ($types as $type) {
            if (isset($data[$type])) {
                ...
                if ($written) {
                    ...
                    $results['errors'][] = sprintf(__('Failed to write %s.json', 'spm-sync'), $type);
                }
            }
        }
        ...
        if (isset($data['sync_version'])) {
            update_option('spm_sync_version', $data['sync_version']);
        }

        // Write sync meta file
        $this->write_json_file('sync_meta.json', array(
            'version' => get_option('spm_sync_version', 0),
            'synced_at' => $results['timestamp'],
            'synced_types' => $results['synced'],
        ));

        // Update last sync time
        update_option('spm_last_sync', $results['timestamp']);
    ...
    private function write_json_file($filename, $data) {
        $file_path = SPM_SYNC_DATA_DIR . $filename;
    ...
    public function get_local_version() {
        return (int) get_option('spm_sync_version', 0);
    }
    ...
    private function log_sync($results) {
        $log_file = SPM_SYNC_DATA_DIR . 'sync.log';
    ...
    public function ajax_manual_sync() {
        // Verify nonce
        if (!check_ajax_referer('spm_sync_nonce', 'nonce', false)) {
            wp_send_json_error(array('message' => __('Security check failed', 'spm-sync')));
        }

        // Check permissions
        if (!current_user_can('manage_options')) {
            wp_send_json_error(array('message' => __('Permission denied', 'spm-sync')));
        }
        ...
        if ($results['success']) {
            wp_send_json_success(array(
                'message' => sprintf(
                    __('Sync completed. Updated: %s', 'spm-sync'),
                    implode(', ', $results['synced'])
                ),
                'results' => $results,
            ));
    ...
    public function ajax_check_version() {
        ...
        wp_send_json_success(array(
            ...
            'last_sync' => get_option('spm_last_sync'),
        ));
    }
    ...
    public function get_status() {
        ...
        foreach ($data_files as $file) {
            $path = SPM_SYNC_DATA_DIR . $file . '.json';
        ...
        return array(
            ...
            'last_sync' => get_option('spm_last_sync'),
            'data_dir' => SPM_SYNC_DATA_DIR,
            'data_url' => SPM_Sync_Plugin::get_data_url(),
            'files' => $file_status,
        );
    }
}
