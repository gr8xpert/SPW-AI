...
class SPW_OG_Tags {
    ...
    public function __construct() {
        ...
        if (get_option('spw_enable_og_tags', true)) {
    ...
    private function is_property_page() {
        ...
        $pattern = get_option('spw_property_url_pattern', '/property/{reference}/');
        ...
        if (isset($_GET['spw_property'])) {
            $this->load_property(sanitize_text_field($_GET['spw_property']));
    ...
    private function load_property($identifier) {
        ...
        $dashboard_url = rtrim(get_option('spw_dashboard_url', ''), '/');
        ...
        $cache_key = 'spw_property_' . md5($identifier);
    ...
    public function render_og_tags() {
        ...
        $title = $this->get_localized_text($property, 'title', $lang_code);
    ...
    public function filter_document_title($title) {
        ...
        $prop_title = $this->get_localized_text($property, 'title', $lang_code);
    ...
    public function filter_title($title_parts) {
        ...
        $title_parts['title'] = $this->get_localized_text($property, 'title', $lang_code);
    ...
    private function format_price($property) {
        if (!empty($property['priceOnRequest']) || empty($property['price'])) {
            return __('Price on request', 'spw-sync');
        }
